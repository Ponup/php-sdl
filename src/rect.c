...
zend_bool sdl_rect_to_zval(SDL_Rect *rect, zval *value)
{
	if (rect) {
		...
		zend_update_property_long(php_sdl_rect_ce, PHP7to8_OBJ_PROP(value), "x", 1, rect->x);
		zend_update_property_long(php_sdl_rect_ce, PHP7to8_OBJ_PROP(value), "y", 1, rect->y);
		zend_update_property_long(php_sdl_rect_ce, PHP7to8_OBJ_PROP(value), "w", 1, rect->w);
		zend_update_property_long(php_sdl_rect_ce, PHP7to8_OBJ_PROP(value), "h", 1, rect->h);
...
zend_bool sdl_point_to_zval(SDL_Point *pt, zval *value)
{
	if (pt) {
		...
		zend_update_property_long(php_sdl_point_ce, PHP7to8_OBJ_PROP(value), "x", 1, pt->x);
		zend_update_property_long(php_sdl_point_ce, PHP7to8_OBJ_PROP(value), "y", 1, pt->y);
...
zend_bool zval_to_sdl_rect(zval *value, SDL_Rect *rect)
{
	if (instanceof_function(Z_OBJCE_P(value), php_sdl_rect_ce)) {
		...
		val = zend_read_property(php_sdl_rect_ce, PHP7to8_OBJ_PROP(value), "x", 1, 0, &rv);
		convert_to_long(val);
		Z_LVAL_P(val) = rect->x = (int)Z_LVAL_P(val);

		val = zend_read_property(php_sdl_rect_ce, PHP7to8_OBJ_PROP(value), "y", 1, 0, &rv);
		convert_to_long(val);
		Z_LVAL_P(val) = rect->y = (int)Z_LVAL_P(val);

		val = zend_read_property(php_sdl_rect_ce, PHP7to8_OBJ_PROP(value), "w", 1, 0, &rv);
		convert_to_long(val);
		Z_LVAL_P(val) = rect->w = (int)Z_LVAL_P(val);

		val = zend_read_property(php_sdl_rect_ce, PHP7to8_OBJ_PROP(value), "h", 1, 0, &rv);
		convert_to_long(val);
...
zend_bool zval_to_sdl_point(zval *value, SDL_Point *pt)
{
	if (instanceof_function(Z_OBJCE_P(value), php_sdl_point_ce)) {
		...
		val = zend_read_property(php_sdl_point_ce, PHP7to8_OBJ_PROP(value), "x", 1, 0, &rv);
		convert_to_long(val);
		Z_LVAL_P(val) = pt->x = (int)Z_LVAL_P(val);

		val = zend_read_property(php_sdl_point_ce, PHP7to8_OBJ_PROP(value), "y", 1, 0, &rv);
		convert_to_long(val);
...
static PHP_METHOD(SDL_Rect, __construct)
{
	...
	zend_update_property_long(php_sdl_rect_ce, PHP7to8_OBJ_PROP(getThis()), "x", 1, x);
	zend_update_property_long(php_sdl_rect_ce, PHP7to8_OBJ_PROP(getThis()), "y", 1, y);
	zend_update_property_long(php_sdl_rect_ce, PHP7to8_OBJ_PROP(getThis()), "w", 1, w);
	zend_update_property_long(php_sdl_rect_ce, PHP7to8_OBJ_PROP(getThis()), "h", 1, h);
}
/* }}} */
...
static PHP_METHOD(SDL_Point, __construct)
{
	...
	zend_update_property_long(php_sdl_point_ce, PHP7to8_OBJ_PROP(getThis()), "x", 1, x);
	zend_update_property_long(php_sdl_point_ce, PHP7to8_OBJ_PROP(getThis()), "y", 1, y);
}
/* }}} */
